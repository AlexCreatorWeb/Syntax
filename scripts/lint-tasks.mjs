...
for (const f of files) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (!/^[a-z]+-\d{3}$/.test(t.id || "")) err(f, `${id}: id не в формате {track}-{NNN}`);
    if (seenIds.has(t.id)) err(f, `${id}: id уже занят`);
    seenIds.add(t.id);
    // track
    if (t.track !== track) err(f, `${id}: track (${t.track}) не совпадает с файлом (${track})`);
    // локализация: en + ru обязательны
    for (const k of ["title", "prompt", "hint"]) {
      if (!t[k] || !t[k].en || !t[k].ru) err(f, `${id}: ${k} должен иметь en + ru`);
    }
    // difficulty / xp
    if (!DIFFS.includes(t.difficulty)) err(f, `${id}: difficulty (${t.difficulty})`);
    else if (t.xp !== XP[t.difficulty]) err(f, `${id}: xp ${t.xp} ≠ ${XP[t.difficulty]} для ${t.difficulty}`);
    // files
    if (!t.files || Object.keys(t.files).length === 0) err(f, `${id}: files пуст`);
    // tests
    if (!Array.isArray(t.tests) || t.tests.length < 2) err(f, `${id}: tests — 2..5 штук`);
    else if (t.tests.length > 5) err(f, `${id}: tests — 2..5 штук`);
    else t.tests.forEach((x, j) => {
    ...
    if (t.lessonId && !/^[a-z]+-\d{2,3}$/.test(t.lessonId)) err(f, `${id}: lessonId не в формате {track}-NN`);
    total += 1;
  }
...
console.log(`Сложность: easy ${diffCount.easy} / medium ${diffCount.medium} / hard ${diffCount.hard}`);
console.log(`DailyChallenge: ${dailyTotal} (${Math.round((dailyTotal / Math.max(1, total)) * 100)}%)`);
for (const [k, v] of Object.entries(trackStats)) console.log(`  ${k}: ${v.n} задач, ${v.cats} кат.`);
