...
const SCOPES = {
  global: [
    { name: "DevJedi", level: 42, streak: 14, hot: true, tasks: 342, xp: 10850, delta: 2, hue: 200 },
    { name: "CodeNinja", level: 40, streak: 3, hot: false, tasks: 315, xp: 10120, delta: -1, hue: 260 },
    { name: "you", level: 38, streak: 7, hot: true, tasks: 289, xp: 9840, delta: 2, hue: 158, you: true },
  ],
  friends: [
    { name: "CodeNinja", level: 40, streak: 3, hot: false, tasks: 315, xp: 10120, delta: 0, hue: 260 },
    { name: "PixelWitch", level: 35, streak: 11, hot: true, tasks: 240, xp: 8975, delta: 1, hue: 300 },
    { name: "you", level: 38, streak: 7, hot: true, tasks: 289, xp: 9840, delta: 2, hue: 158, you: true },
  ],
  teams: [
    { name: "NullPtr Co.", members: 5, streak: 21, hot: true, tasks: 1240, xp: 12840, delta: 1, hue: 200 },
    { name: "Syntax Squad", members: 4, streak: 12, hot: true, tasks: 860, xp: 9840, delta: 2, hue: 158, you: true },
  ],
};
...
const TECH_FACTOR = { all: 1, html: 0.55, css: 0.6, javascript: 0.95, python: 0.8 };
...
function Delta({ value }) {
  if (value > 0) return <span className="rk-delta rk-delta--up" aria-hidden="true">▲{value}</span>;
  if (value < 0) return <span className="rk-delta rk-delta--down" aria-hidden="true">▼{-value}</span>;
...
function RankingsView({ isAuthed = false, onAuth = null }) {
  ...
  useEffect(() => {
    ...
    const close = (e) => {
      if (howRef.current && !howRef.current.contains(e.target)) setHowOpen(false);
    };
  ...
  const rowsAll = SCOPES[scope]
    .map((r) => ({ ...r, xpNow: Math.round(r.xp * PERIOD_MULT[period] * TECH_FACTOR[techFilter]) }))
  ...
  const nextRow = youRow ? rowsAll.find((r) => r.rank === youRow.rank - 1) : null;
  const youGap = nextRow ? nextRow.xpNow - youRow.xpNow : 0;
  const youProgress = nextRow ? Math.min(100, Math.round((youRow.xpNow / nextRow.xpNow) * 100)) : 0;
        ...
        <div className="rankings__head-left">
          <h1 className="rankings__title">
            ...
            <span className="rankings__how" ref={howRef}>
              ...
              {howOpen && (
                <div className="rankings__how-pop card" role="dialog" aria-label={t("rankings.howTitle")}>
          ...
          <div className="rankings__segs">
            <div className="seg" role="tablist" aria-label={t("rankings.titleScope", { scope: t(`rankings.${scope}`) })}>
            ...
            <div className="seg" role="tablist" aria-label={t("rankings.periodAll")}>
              {[["week", "rankings.periodWeek"], ["month", "rankings.periodMonth"], ["all", "rankings.periodAll"]].map(
                ([p, key]) => (
                  <button
                    key={p}
                    type="button"
        ...
        {PODIUM.map((user) => (
          <div key={user.rank} className={`podium__slot podium__slot--${user.rank}`}>
            ...
            <p className="podium__meta">
              <span className={`streak ${user.streak >= 7 ? "streak--hot" : ""}`}>
                <FlameIcon hot={user.streak >= 7} />
                {user.streak}
              </span>
              <Delta value={user.delta} />
            </p>
            <div className={`podium__pedestal podium__pedestal--${user.rank}`} />
          </div>
        ))}
      </section>
        ...
        <div className="rankings__table-scroll">
            ...
            <tbody key={`${scope}-${period}-${techFilter}-${query}`}>
              {rows.map((row) => (
                ...
                >
                  ...
                  <td>
                    ...
                    {row.you && nextRow && isAuthed && (
                      <div className="rankings__you-next">
                        ...
                        <span>{t("rankings.rowNext", { n: fmt(youGap), rank: nextRow.rank })}</span>
                      </div>
                    )}
                  </td>
                  ...
                  <td className="rk-right rankings__tasks col-tasks">{fmt(row.tasks)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {rows.length === 0 && (
            <div className="tasks-empty rankings__empty">
              {t("tasks.noResults")}
              {query && (
                <button type="button" className="btn btn--ghost rankings__showme" onClick={() => setQuery("")}>
                  {t("rankings.showMe")}
                </button>
              )}
            </div>
