...
const withTimeout = (p, ms = 4000) =>
  Promise.race([p, new Promise((res) => setTimeout(() => res({ data: null, error: { message: "timeout" } }), ms))]);
...
export function pushProgressToDb() {
  ...
  const lessonRows = [];
  TECHS.forEach(({ id }) => {
    getCompleted(id, uid).forEach((lessonId) => lessonRows.push({ user_id: uid, lesson_id: lessonId }));
  });
  if (lessonRows.length) {
    withTimeout(supabase.from("lesson_progress").upsert(lessonRows, { onConflict: "user_id,lesson_id" }))
      .then((r) => r.error && console.warn("db-progress: lesson upsert", r.error.message))
      .catch(() => {});
  }
  const taskRows = [];
  ...
  if (taskRows.length) {
    withTimeout(supabase.from("task_progress").upsert(taskRows, { onConflict: "user_id,task_id" }))
      .then((r) => r.error && console.warn("db-progress: task upsert", r.error.message))
      .catch(() => {});
  }
}
...
export function pushLessonComplete(lessonId) {
  ...
  withTimeout(supabase.from("lesson_progress").upsert([{ user_id: uid, lesson_id: lessonId }], { onConflict: "user_id,lesson_id" }))
    .then((r) => r.error && console.warn("db-progress: lesson row", r.error.message))
    .catch(() => {});
}
...
export function pushTaskComplete(track, taskId, xp = 0) {
  ...
  withTimeout(
    supabase.from("task_progress").upsert([{ user_id: uid, task_id: taskId, xp }], { onConflict: "user_id,task_id" })
  )
    .then((r) => r.error && console.warn("db-progress: task row", r.error.message))
    .catch(() => {});
}
